...
import { Send, MessageSquare, Smartphone, Users, AlertCircle } from 'lucide-react';
...
export default function WhatsAppPage() {
  ...
  const allMessages = [...messages.map(m => ({...m, channel: m.channel || 'whatsapp'})), ...smsMessages.map(m => ({...m, channel: 'sms'}))].sort((a, b) => (b.sent_at || '').localeCompare(a.sent_at || ''));
    ...
    <div data-testid="whatsapp-page" className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-0.5">Send WhatsApp messages and SMS to customers</p>
        </div>
      </div>

      <Tabs defaultValue="sms">
        <TabsList>
          <TabsTrigger data-testid="tab-sms" value="sms" className="gap-1.5"><Smartphone className="w-3.5 h-3.5" /> SMS (Fast2SMS)</TabsTrigger>
          <TabsTrigger data-testid="tab-whatsapp" value="whatsapp" className="gap-1.5"><MessageSquare className="w-3.5 h-3.5" /> WhatsApp (Mock)</TabsTrigger>
        ...
        <TabsContent value="whatsapp" className="mt-4">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-lg">
                      <div className="w-1.5 h-1.5 bg-amber-400 rounded-full" /><span className="text-[10px] font-medium text-amber-700">MOCK</span>
                    </div>
                  </div>
                  <Button data-testid="wa-bulk-send-btn" disabled={sending || selectedIds.length === 0} onClick={() => setConfirmBulk({ type: 'whatsapp', count: selectedIds.length })} style={{ backgroundColor: 'var(--brand-primary)' }}>
                    <Send className="w-4 h-4 mr-1.5" /> Send to {selectedIds.length}
                  </Button>
                </div>
                <Table>
                  ...
                  <TableBody>
                    {filteredCustomers.map(c => (
                      <TableRow key={c.id}><TableCell><Checkbox checked={selectedIds.includes(c.id)} onCheckedChange={() => toggleSelect(c.id)} /></TableCell>
            ...
            <div className="lg:col-span-2">
              <div className="bg-white border border-neutral-200 rounded-lg">
                <div className="p-4 border-b border-neutral-200"><h3 className="text-sm font-semibold text-neutral-900" style={{ fontFamily: 'Outfit' }}>WhatsApp History</h3></div>
                <div className="max-h-[500px] overflow-y-auto divide-y divide-neutral-100">
                  {messages.length === 0 ? <div className="text-center py-12 text-neutral-400"><p className="text-sm">No messages yet</p></div> :
                  messages.slice(0, 20).map(m => (
                    <div key={m.id} className="p-3">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-medium text-neutral-700">{m.customer_name}</span>
                        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${m.status === 'delivered' ? 'bg-emerald-50 text-emerald-600' : 'bg-neutral-100 text-neutral-500'}`}>{m.status}</span>
                      </div>
        ...
        <TabsContent value="history" className="mt-4">
          <div className="bg-white border border-neutral-200 rounded-lg">
            <div className="p-4 border-b border-neutral-200"><h3 className="text-sm font-semibold text-neutral-900" style={{ fontFamily: 'Outfit' }}>All Message History</h3></div>
            <Table>
              <TableHeader><TableRow className="bg-neutral-50">
              ...
              <TableBody>
                {allMessages.slice(0, 50).map(m => (
                  <TableRow key={m.id}>
                    <TableCell>
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${m.channel === 'sms' ? 'bg-blue-50 text-blue-700' : 'bg-green-50 text-green-700'}`}>
      ...
      <Dialog open={!!confirmBulk} onOpenChange={() => setConfirmBulk(null)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button data-testid="confirm-bulk-send-btn" onClick={confirmBulk?.type === 'sms' ? handleSmsBulkSend : handleWaBulkSend} disabled={sending} style={{ backgroundColor: 'var(--brand-primary)' }}>
